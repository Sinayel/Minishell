...
/*   Updated: 2024/11/27 18:07:29 by ylouvel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_magouilles(char *str1, char *str2, char *str3)
{
	...
	i = 0;
	j = 0;
	dest = (char *)malloc(sizeof(char) * (ft_strlen(str1) + ft_strlen(str2)
				+ ft_strlen(str3) + 1));
	while (str1[i])
	{
		dest[i] = str1[i];
...
int	ft_export(t_env *env_list, char *arg)
{
	...
	while (arg_tabtab[i])
	{
		...
		else if (check_equal_arg(arg_tabtab[i]) == 1)
		{
			...
			if (env_value == NULL)
			{
				if (verif_if_in_export(export->content, arg_tabtab[i]) == 1)
					export->content = replace_one_in_export(export->content,
							arg_tabtab[i]);
				else
					export->content = append_to_export(export->content,
							arg_tabtab[i]);
				export_to_env(&env_list, split_arg);
				//! SPLIT PAR RAPPOR A L'EGAL
			}
			else
			{
		...
		else if ((check_equal_arg(arg_tabtab[i]) == 0
				|| check_equal_arg(arg_tabtab[i]) == 2) && env_value == NULL
			&& verif_if_in_export(export->content, arg_tabtab[i]) == 0)
